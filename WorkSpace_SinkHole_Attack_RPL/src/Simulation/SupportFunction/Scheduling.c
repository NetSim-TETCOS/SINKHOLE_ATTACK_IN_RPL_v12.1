...
static enum enum_BUFFER red_algorithm(NetSim_BUFFER* pstruBuffer, NetSim_PACKET* pstruData)
{
		...
		queuing_var->avg_queue_size < queuing_var->max_th)
	{
		...
		if (queuing_var->random_value < pb / (1 - queuing_var->count * pb))
		{
			queuing_var->count = 0;
...
static enum enum_BUFFER wred_algorithm(NetSim_BUFFER* pstruBuffer, NetSim_PACKET* pstruData, double max_th, double min_th)
{
	...
	{
		queuing_var->count++;
		pb = queuing_var->max_p * (queuing_var->avg_queue_size - min_th) /
			(max_th - min_th);
		if (queuing_var->random_value < pb / (1 - queuing_var->count * pb))
		{
			queuing_var->count = 0;
...
enum enum_BUFFER fn_NetSim_WRED(NetSim_BUFFER* pstruBuffer, NetSim_PACKET* pstruData)
{
	...
	if (checkBuffer == Buffer_Underflow)
	{
		...
		else
		{
			...
			checkBuffer = wred_algorithm(pstruBuffer, pstruData, curr_max_th, curr_min_th);
		}
